...
//Vytvoríme funkciu na tlač matice, aby sme mohli overiť výsledok:
...
//Napíšeme hlavnú funkciu na testovanie všetkých vyššie uvedených funkcií:

int main() {
    int size = 5; // Veľkosť matice
    MAT *mat = mat_init(size, size);

    if (mat_create_random_permutation(mat)) {
        printf("Permutacna matica bola uspesne vytvorena:\n");
        mat_print(mat);
    } else {
        printf("Nepodarilo sa vytvorit permutacnu maticu.\n");
    }
